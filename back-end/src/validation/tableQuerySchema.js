...
const tableQuerySchema = Joi.object({
    tableName: Joi.string().min(3).max(50).optional().messages({
    ...
    tableCapacity: Joi.number().integer().positive().min(1).optional().messages({
    ...
    tableType: Joi.string().valid("Regular", "VIP", "Outdoor").optional().messages({
        "any.only": 'Table type must be one of the following: "Regular", "VIP", or "Outdoor".',
    }),
    tableStatus: Joi.string().valid("Available", "Reserved", "Occupied").optional().messages({
